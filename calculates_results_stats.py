...
def calculates_results_stats(results_dic):
    ...
    Parameters:
        results_dic - Dictionary with key as image filename and value as a List:
            ...
            idx 3 = 1/0 (int)  where 1 = pet image 'is-a' dog, 0 = pet image 'is-NOT-a' dog.
            idx 4 = 1/0 (int)  where 1 = Classifier classifies image 'as-a' dog,
                                   0 = Classifier classifies image 'as-NOT-a' dog.

    Returns:
        results_stats_dic - Dictionary that contains the results statistics, with keys being
                            the statistic's name (starting with 'pct' for percentage or 'n' for count)
                            and the value being the statistic's value.
    """
    ...
    for key, value in results_dic.items():
        ...
        if value[3] == 1:  # Pet image 'is-a' dog
            ...
            if value[4] == 1:  # Classifier classified image 'as-a' dog
                results_stats_dic['n_correct_dogs'] += 1

        else:  # Pet image 'is-NOT-a' dog
            if value[4] == 0:  # Classifier classified image 'as-NOT-a' dog
